...
###### Create Game Layout/Design #########
app = Tk()
app.geometry("1350x750+0+0")
app.title("Tic Tac Toe")
app.configure(background='Cadet Blue')

Top = Frame(app, bg = 'Cadet Blue', pady =2, width =1350, height =100, relief =RIDGE)
Top.grid(row=0, column=0)

label_title = Label(Top, font=('arial', 50,'bold'), text="Advanced Tic Tac Toe Game", bd=21, bg='Cadet Blue', fg='Cornsilk', justify=CENTER)
label_title.grid(row=0, column=0)


Main_frame = Frame(app, bg = 'Yellow', bd =10, width =1350, height =600, relief =RIDGE)
Main_frame.grid(row=1, column=0)

LeftFrame = Frame(Main_frame, bd=10, width =750, height=500, pady=2, padx=10, bg="Cadet Blue", relief=RIDGE)
LeftFrame.pack(side=LEFT)

RightFrame = Frame(Main_frame, bd=10, width =560, height=500, padx=10, pady=2, bg="Cadet Blue", relief=RIDGE)
...
def scoreKeeper():
    ...
    if (button1["text"] == "X" and button4["text"] == "X" and button7["text"] =="X"):
        button1.configure(background = "indigo")
        button4.configure(background = "indigo")
        button7.configure(background = "indigo")
        n = float(PlayerX.get())
        score = (n+1)
        PlayerX.set(score)
        tkinter.messagebox.showinfo("Winner X", "You have just won the game")
    ...
    if (button3["text"] == "X" and button6["text"] == "X" and button9["text"] =="X"):
        ...
        button6.configure(background = "violet")
        button9.configure(background = "violet")
        n = float(PlayerX.get())
        score = (n+1)
        PlayerX.set(score)
        tkinter.messagebox.showinfo("Winner X", "You have just won the game")
...
###### Create and Configure User buttons ########
def reset():
    button1['text'] = " "
    button2['text'] = " "
    button3['text'] = " "
...
label_PlayerX = Label(RightFrame1, font=('arial', 40,'bold'), text="Player X :", padx=2, pady=2, bg='Cadet Blue')
label_PlayerX.grid(row=0, column=0, sticky=W)
textPlayerX=Entry(RightFrame1, font=('arial', 40, 'bold'), bd=2, fg="black", textvariable= PlayerX, width=14, justify=LEFT)
textPlayerX.grid(row=0, column=1)


label_PlayerO = Label(RightFrame1, font=('arial', 40,'bold'), text="Player O:", padx=2, pady=2, bg='Cadet Blue')
label_PlayerO.grid(row=1, column=0, sticky=W)
textPlayerO=Entry(RightFrame1, font=('arial', 40, 'bold'), bd=2, fg="black", textvariable= PlayerO, width=14, justify=LEFT)
textPlayerO.grid(row=1, column=1)


btn_reset = Button(RightFrame2, text=" Reset", font=('arial', 40, 'bold'), height = 1, width =20, command=reset)
btn_reset.grid(row=2, column=0, pady=11, padx=6)
